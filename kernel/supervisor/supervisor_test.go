...
`)
	...
	// no eventually-loop.
	ws1Items, err := idx.StreamItems(ctx, "ws1", nil, nil, 0, 0, index.ViewIncluded)
	if err != nil {
	...
	ws2Items, err := idx.StreamItems(ctx, "ws2", nil, nil, 0, 0, index.ViewIncluded)
	if err != nil {
...
`)
	...
	items, err := idx.StreamItems(ctx, "shell-target", nil, nil, 0, 0, index.ViewIncluded)
	if err != nil {
...
`)
	...
	items, err := idx.StreamItems(ctx, "ws1", nil, nil, 0, 0, index.ViewIncluded)
	if err != nil {
		t.Fatalf("StreamItems(ws1): %v", err)
	}
...
`)
	...
	items, err := idx.StreamItems(ctx, "doomed", nil, nil, 0, 0, index.ViewIncluded)
	if err != nil {
