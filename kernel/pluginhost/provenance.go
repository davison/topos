...
func AcceptedProvenanceKeys() []ProvenanceKey {
	...
	out := make([]ProvenanceKey, 0, len(embeddedProvenanceKeys)+len(provenanceExtraParsed))
	out = append(out, embeddedProvenanceKeys...)
	out = append(out, provenanceExtraParsed...)
	return out
}
...
func readAndVerifyProvenanceManifest(manifestPath string, keyByID map[string]ed25519.PublicKey) (manifest ProvenanceManifest, diagnostic string, ok bool) {
	...
	if err != nil {
		return ProvenanceManifest{}, fmt.Sprintf("%s: cannot stat manifest: %v", fname, err), false
	}
	if info.Size() > MaxProvenanceManifestBytes {
		return ProvenanceManifest{}, fmt.Sprintf("%s: manifest exceeds %d bytes, refusing to read", fname, MaxProvenanceManifestBytes), false
	}
	manifestBytes, err := os.ReadFile(manifestPath)
	if err != nil {
		return ProvenanceManifest{}, fmt.Sprintf("%s: cannot read manifest: %v", fname, err), false
	}

	sigInfo, err := os.Stat(sigPath)
	if err != nil {
		return ProvenanceManifest{}, fmt.Sprintf("%s: missing or unreadable sibling signature file %s: %v", fname, filepath.Base(sigPath), err), false
	}
	if sigInfo.Size() > MaxProvenanceManifestBytes {
		return ProvenanceManifest{}, fmt.Sprintf("%s: sibling signature file exceeds %d bytes, refusing to read", fname, MaxProvenanceManifestBytes), false
	}
	sigBytes, err := os.ReadFile(sigPath)
	if err != nil {
		return ProvenanceManifest{}, fmt.Sprintf("%s: cannot read sibling signature file %s: %v", fname, filepath.Base(sigPath), err), false
	}

	var sig ProvenanceSignature
	if err := json.Unmarshal(sigBytes, &sig); err != nil {
		return ProvenanceManifest{}, fmt.Sprintf("%s: sibling signature file %s is malformed JSON: %v", fname, filepath.Base(sigPath), err), false
	}
	if sig.Algorithm != "ed25519" {
		return ProvenanceManifest{}, fmt.Sprintf("%s: signature algorithm %q is not \"ed25519\"", fname, sig.Algorithm), false
	}
	pub, known := keyByID[sig.KeyID]
	if !known {
		return ProvenanceManifest{}, fmt.Sprintf("%s: signature names unknown key id %q", fname, sig.KeyID), false
	}
	sigRaw, err := base64.StdEncoding.DecodeString(sig.Signature)
	if err != nil || len(sigRaw) != ed25519.SignatureSize {
		return ProvenanceManifest{}, fmt.Sprintf("%s: signature is not a valid base64-encoded ed25519 signature", fname), false
	}
	if !ed25519.Verify(pub, manifestBytes, sigRaw) {
		return ProvenanceManifest{}, fmt.Sprintf("%s: signature does not verify against key %q — manifest bytes do not match what was signed", fname, sig.KeyID), false
	}

	if err := json.Unmarshal(manifestBytes, &manifest); err != nil {
		return ProvenanceManifest{}, fmt.Sprintf("%s: manifest is malformed JSON despite a valid signature", fname), false
	}
	if manifest.Schema != ProvenanceSchema {
		return ProvenanceManifest{}, fmt.Sprintf("%s: manifest schema %q is not %q", fname, manifest.Schema, ProvenanceSchema), false
	}
	if manifest.Release.OS != runtime.GOOS || manifest.Release.Arch != runtime.GOARCH {
		return ProvenanceManifest{}, fmt.Sprintf("%s: manifest release os/arch %s/%s does not match this kernel's %s/%s", fname, manifest.Release.OS, manifest.Release.Arch, runtime.GOOS, runtime.GOARCH), false
	}
...
func VerifySignedProvenance(dirs Dirs, name, path string) (hash string, evidence string, diagnostics []string, err error) {
	...
	keyByID := make(map[string]ed25519.PublicKey, len(acceptedKeys))
	for _, k := range acceptedKeys {
		keyByID[k.ID] = k.PublicKey
	}
	...
	for _, dir := range []string{dirs.Trusted, dirs.External} {
		...
		for _, e := range entries {
			...
			manifest, diag, ok := readAndVerifyProvenanceManifest(manifestPath, keyByID)
			if !ok {
	...
	switch {
	case matchFile != "":
		return hash, matchFile, diagnostics, nil
	case haveMismatch:
		return hash, "", diagnostics, fmt.Errorf("%w: %q — manifest %s names a digest that does not match the on-disk binary (signed digest %s, on-disk %s)", ErrProvenanceUnverified, name, mismatchFile, mismatchDigest, hash)
	default:
		return hash, "", diagnostics, nil
	}
}
...
func EvaluateTrust(dirs Dirs, name, path string) (Trust, error) {
	...
	return Trust{Tier: TierExternal, Hash: provHash, Diagnostics: diagnostics}, nil
}
