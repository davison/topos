...
provides:
  ...
  - "14-LIVE-UAT.md — the written, not-yet-run live UAT script for the three criteria only a real Google account can prove"
...
coverage:
  ...
  - id: D2
    ...
    human_judgment: false
  - id: D3
    description: "Task 2's human-check: running 14-LIVE-UAT.md end to end against a real Google account and filling in its results table — this is the plan's own <human-check> verify block, not yet run"
    verification: []
    human_judgment: true
    rationale: "Requires a real Google Cloud OAuth client, a real Google Drive account, and a real browser-driven OAuth consent flow — none of which this worktree executor can fabricate or should ever be handed (the two credential values must never reach an automated agent). This is the one remaining checkpoint blocking this plan's completion."

# Metrics
duration: ~2h (includes live root-cause investigation of a real test failure against the real binary)
...
**A five-test hermetic Playwright spec proves the real, out-of-repo `topos-plugin-gdrive` binary on the untrusted external path (external tier, exact not-authorized health sentence, generic Describe with zero code in this repository naming the plugin, trust badge + accessible-description disclosure); the live UAT script for the three criteria only a real Google account can prove is written but not yet run — that run is the one remaining human-only checkpoint.**

## Performance

- **Duration:** ~2h (Task 1 authoring + a substantial live root-cause investigation when the first real-binary run surfaced two genuine test-authoring defects; Task 2's write half)
- **Tasks:** 1.5 of 2 (Task 1 complete; Task 2's write half complete, its human-check run half pending)
...
**Task 2's run half** — running `14-LIVE-UAT.md` against a real Google account and filling in its results table — is the plan's own `<human-check>` verify block. It has not been run; this is a checkpoint, not a deferred task.
...
- `.planning/phases/14-google-drive-source-built-out-of-repo/14-LIVE-UAT.md` (new) — the live UAT script described above.
...
**Total deviations:** 2 auto-fixed (both Rule 1 — genuine test-authoring bugs surfaced only by running against the real binary, not plan defects; the plan's own action text did not specify a webspace shape or the exact assertion form, leaving both decisions to this execution). No sibling-repository file was read, written, or modified beyond read-only source inspection used to rule out a plugin-side defect (`plugin.go`, `token.go`, `plugin_test.go` — all read-only, per this plan's own read-only boundary on `/home/darren/projects/davison/topos-plugin-gdrive`).
**Impact on plan:** Both fixes were necessary for Task 1's own hard acceptance criterion ("with the variable set to a real binary, the new target runs and all five tests in the spec pass") to hold at all. No scope creep — no code outside the new spec file was touched.

## Issues Encountered

Root-causing the two real-binary test failures took most of this session's time: the `last_error`-empty symptom initially looked exactly like a plugin defect (the built binary silently returning a healthy-but-empty stream instead of the named health sentence — precisely the failure mode this plan's own must_have truth exists to catch), and an early false lead (a genuine, pre-existing, correctly-dated token file at the operator's real `~/.local/share/topos-plugin-gdrive/token.json` — left over from Phase 14-03's own real-account authorization) briefly suggested a HOME-isolation leak in the hermetic harness. An env-dumping wrapper script around the real binary conclusively ruled that out (`HOME` was correctly the fixture's own temp directory), which is what redirected the investigation to `kernel/correlate/correlate.go`'s participation gate — the actual cause. No topos-plugin-gdrive code was found to be at fault; both defects were in this plan's own test authoring.

## User Setup Required

**Task 2's human-check verify block is the one remaining checkpoint.** Per this plan's own `user_setup` and `14-LIVE-UAT.md`'s Preparation section, the operator needs:
- A Google Cloud project with the Drive API enabled and a Desktop-app OAuth client, published to Production status (not Testing — see `14-LIVE-UAT.md` step 7's explanation of the 7-day silent-expiry consequence).
- `GDRIVE_CLIENT_ID`/`GDRIVE_CLIENT_SECRET` exported into the shell that runs the auth CLI and starts the kernel — never written into any file.
- A target Drive folder's id (from its Drive URL) for the `folder_id` field.

Then: run `.planning/phases/14-google-drive-source-built-out-of-repo/14-LIVE-UAT.md` end to end and fill in its results table. This is genuinely a human-only step — no automation can fabricate a Google account, a real OAuth consent flow, or the operator's own credential values, and those values must never reach an automated agent.
...
- `14-LIVE-UAT.md` is written and its automated `<verify>` half (`test -s ... && make docs-check`) passes. Its human-check half is unresolved — this plan cannot be marked `status: complete` until the operator runs it and the orchestrator (or a continuation session) records the filled-in results and resolves the checkpoint, matching the pattern `14-03-SUMMARY.md` already established for its own Task 3 hand-off checkpoint.
- Plan 14-05 (gap triage) depends on this plan only for the general Phase 14 UAT context, not on Task 2's specific run — it is not blocked by this checkpoint.
- STATE.md, ROADMAP.md, and REQUIREMENTS.md are deliberately **not** updated by this execution (worktree mode; the orchestrator owns those writes after all wave agents complete) — and `requirements-completed` is deliberately left empty in this SUMMARY's own frontmatter, since SRC-05/SRC-06 are not yet fully proven end to end pending Task 2's human-check.

---
*Phase: 14-google-drive-source-built-out-of-repo*
*Completed: 2026-08-18 (Task 1 complete and verified; Task 2 write-half complete; Task 2 run-half — the live UAT against a real Google account — is the checkpoint blocking full completion)*
