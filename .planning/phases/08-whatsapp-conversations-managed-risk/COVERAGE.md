...
| `session-persist` — `sqlstore.Container`, `GetFirstDevice`, auto-save on pair | INTEGRATE | |
...
| `events-session` — `LoggedOut`, `StreamReplaced`, `Connected`, temp-ban events | INTEGRATE | |
...
| `send-message` — `SendMessage`, `SendReaction`, `MarkRead`, other mutate calls | OPT-OUT | explicitly out of scope (incl. `SendChatPresence`, `RevokeMessage`, `EditMessage`) — `docs/plugin-contract.md`'s read-only-by-construction rule; enforced by Plan 08-02 Task 3's AST scan |
...
| `groups-mutate` — `CreateGroup`, `JoinGroupWithLink`, `LeaveGroup`, and peers | OPT-OUT | explicitly out of scope (incl. `UpdateGroupParticipants`, `SetGroupName`/`Topic`) — read-only plugin; mutating the user's WhatsApp groups is never a source-plugin action |
| `newsletters` — WhatsApp Channels subscribe/info surface | OPT-OUT | not needed — `NewsletterSubscribe`/`Unsubscribe`/`GetNewsletterInfo`; SRC-03 as widened by D-05 scopes matching to groups and 1:1 chats; channels are neither |
