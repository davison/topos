...
*Added 2026-08-11 (gap closure G-08-4, Plan 08-11): the "Connecting" row above closes a gap where a freshly launched, already-paired instance could report the "Not yet linked" cause instead of an honest transient state. Unlike the other four rows, "Connecting" is transient and self-clearing — it offers no re-link/recovery action because there is nothing for the user to do; the state clears itself once the login handshake completes. Plan 08-11's bounded serve-mode login wait (`serveLoginTimeout`) means the kernel normally never observes this state at all — it exists to make the plugin's self-reported health honest at every instant, not to be a common steady state.
