...
| A PNG/data-URI QR encoder (e.g. `skip2/go-qrcode`) | — | Not checked this session | — | — | UNVERIFIED | **Not yet audited.** If the plan selects the in-app/kernel-mediated QR option, the plan's own research/spike step must run this same manual Go-ecosystem verification against whichever specific package it selects before adding it. |
