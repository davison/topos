...
| Connecting | A paired device is found and the socket is up, but WhatsApp has not yet completed the login handshake | `destructive` | Existing health dot + tooltip, `last_error` verbatim |
...
*Added 2026-08-11 (gap closure G-08-4, Plan 08-11): the "Connecting" row above closes a gap where a freshly launched, already-paired instance could report the "Not yet linked" cause instead of an honest transient state. Unlike the other four rows, "Connecting" is transient and self-clearing — it offers no re-link/recovery action because there is nothing for the user to do; the state clears itself once the login handshake completes.*

*Corrected 2026-08-11 (gap closure G-08-5, Plan 08-14): the sentence this amendment replaces asserted that Plan 08-11's bounded serve-mode login wait (`serveLoginTimeout`) means the kernel normally never observes the connecting state. That is no longer true — Plan 08-14 moved the wait off the launch path (closing 08-REVIEW.md WR-01, a blocking wait there delayed the go-plugin handshake and, through it, kernel boot and every relaunch for every other source), so the kernel CAN legitimately observe the connecting state on a first `Match` after a launch. What now stops that from pinning a failed sync for the full interval is the kernel-side bounded first-refresh retry (`kernel/syncer/scheduler.go`'s `defaultFirstRefreshRetryDelays`), which supersedes a connecting-window `Match` with an ok sync run within seconds.
